...
CR = 0.9;
maxGen = 500;
F_list = [0.1, 0.3, 0.5, 0.7, 0.9, 1.1];  % 取值覆盖从强局部开发到强全局勘探的策略空间
run_times = 10;

mean_curves = zeros(length(F_list), maxGen);
final_vals = zeros(length(F_list), run_times);
converge_iters = zeros(length(F_list), run_times);
...
for f_idx = 1:length(F_list)
    ...
    for run = 1:run_times
        ...
        final_vals(f_idx, run) = best_curve(end);
        converge_iters(f_idx, run) = find(best_curve > 0.99 * best_curve(end), 1);
    end

    mean_curves(f_idx, :) = mean(curves);
end

% ========== 绘制对比收敛曲线 ==========
figure;
hold on;
colororder(lines(length(F_list)))
...
ylabel('平均最优目标函数值');
title('不同变异因子F下的收敛曲线');
legend(arrayfun(@(f) sprintf('F=%.1f', f), F_list, 'UniformOutput', false));
grid on;
hold off;
...
fprintf('\nF值\t均值\t\t标准差\t平均收敛代数\n');
for i = 1:length(F_list)
    fprintf('%.1f\t%.2f\t%.2f\t%.2f\n', F_list(i), mean_final(i), std_final(i), mean_conv(i));
end
